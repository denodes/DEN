...
{
    // The wallet only allows publicSpend inputs in the same tx and not a combination between den and zden
    for(const CTxIn& txin : vin) {
...
CAmount CTransaction::GetValueOut() const
{
    ...
    {
        // DENNODES: previously MoneyRange() was called here. This has been replaced with negative check and boundary wrap check.
        if (it->nValue < 0)
