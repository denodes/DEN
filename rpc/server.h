...
/**
 * DENNODES RPC command dispatcher.
 */
class CRPCTable
{
private:
...
extern UniValue DoZdenSpend(const CAmount nAmount, bool fMintChange, bool fMinimizeChange, vector<CZerocoinMint>& vMintsSelected, std::string address_str, bool isPublicSpend = true);
...
extern UniValue setzdenseed(const UniValue& params, bool fHelp);
extern UniValue getzdenseed(const UniValue& params, bool fHelp);
extern UniValue generatemintlist(const UniValue& params, bool fHelp);
extern UniValue searchdzden(const UniValue& params, bool fHelp);
extern UniValue dzdenstate(const UniValue& params, bool fHelp);
