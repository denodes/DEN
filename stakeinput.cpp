...
// The zDEN block index is the first appearance of the accumulator checksum that was used in the spend
...
{
    //The unique identifier for a zDEN is a hash of the serial
...
{
    //Create an output returning the zDEN that was staked
    CTxOut outReward;
    libzerocoin::CoinDenomination denomStaked = libzerocoin::AmountToZerocoinDenomination(this->GetValue());
    CDeterministicMint dMint;
    if (!pwallet->CreateZDENOutPut(denomStaked, outReward, dMint))
        return error("%s: failed to create zDEN output", __func__);
    vout.emplace_back(outReward);

    //Add new staked denom to our wallet
    if (!pwallet->DatabaseMint(dMint))
        return error("%s: failed to database the staked zDEN", __func__);

    for (unsigned int i = 0; i < 3; i++) {
        CTxOut out;
        CDeterministicMint dMintReward;
        if (!pwallet->CreateZDENOutPut(libzerocoin::CoinDenomination::ZQ_ONE, out, dMintReward))
            return error("%s: failed to create zDEN output", __func__);
...
{
    CzDENTracker* zdenTracker = pwallet->zdenTracker.get();
    CMintMeta meta;
    if (!zdenTracker->GetMetaFromStakeHash(hashSerial, meta))
        return error("%s: tracker does not have serialhash", __func__);

    zdenTracker->SetPubcoinUsed(meta.hashPubcoin, txid);
    return true;
}
...
{
    //The unique identifier for a DEN stake is the outpoint
