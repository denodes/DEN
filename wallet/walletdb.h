...
public:
    ...
    bool WriteZDENSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZDENSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZDENSeed_deprecated(uint256& seed);
    bool EraseZDENSeed();
    bool EraseZDENSeed_deprecated();

    bool WriteZDENCount(const uint32_t& nCount);
    bool ReadZDENCount(uint32_t& nCount);
