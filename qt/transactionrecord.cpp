...
QList<TransactionRecord> TransactionRecord::decomposeTransaction(const CWallet* wallet, const CWalletTx& wtx)
{
    ...
    if (wtx.HasZerocoinSpendInputs()) {
        ...
        if (wtx.HasZerocoinPublicSpendInputs()) {
            ...
            if (!ZDENModule::ParseZerocoinPublicSpend(wtx.vin[0], wtx, state, publicSpend)){
    ...
    if (wtx.IsCoinStake()) {
        ...
        if (wtx.HasZerocoinSpendInputs() && (fZSpendFromMe || wallet->zdenTracker->HasMintTx(hash))) {
            //zDEN stake reward
            sub.involvesWatchAddress = false;
            sub.type = TransactionRecord::StakeZDEN;
...
bool IsZDENType(TransactionRecord::Type type)
{
    switch (type) {
        case TransactionRecord::StakeZDEN:
...
void TransactionRecord::updateStatus(const CWalletTx& wtx)
{
    ...
    else if (type == TransactionRecord::Generated || type == TransactionRecord::StakeMint || type == TransactionRecord::StakeZDEN || type == TransactionRecord::MNReward) {
