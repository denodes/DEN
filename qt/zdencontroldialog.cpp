...
#include "ui_zdencontroldialog.h"
...
void ZPivControlDialog::updateList()
{
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        {
            LOCK(pwalletMain->zdenTracker->cs_spendcache);

            CoinWitnessData *witnessData = pwalletMain->zdenTracker->GetSpendCache(mint.hashStake);
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to precompute or spend zDEN.";
            else if (!mint.isSeedCorrect)
                strReason = "The zDEN seed used to mint this zDEN is not the same as currently hold in the wallet";
            else
