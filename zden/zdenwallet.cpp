...
CzDENWallet::CzDENWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zden seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZDENSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZDENSeed_deprecated()) {
                    LogPrintf("%s: Updated zDEN seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zden seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zden wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzDENWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZDENCount(nCountLastUsed);
    else if (!walletdb.ReadZDENCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzDENWallet::Lock()
{
    seedMaster = 0;
}

void CzDENWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzDENWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZDEN(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzDENWallet::LoadMintPoolFromDB()
{
...
void CzDENWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzDENWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzDENWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zdenTracker->HasPubcoinHash(pMint.first)) {
...
bool CzDENWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZDEN(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zdenTracker which also adds to database
    pwalletMain->zdenTracker->Add(dMint, true);
...
void CzDENWallet::SeedToZDEN(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzDENWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzDENWallet::GenerateDeterministicZDEN(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzDENWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzDENWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzDENWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
