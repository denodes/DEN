...
CzDENTracker::CzDENTracker(std::string strWalletFile)
{
...
CzDENTracker::~CzDENTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzDENTracker::Init()
{
...
bool CzDENTracker::Archive(CMintMeta& meta)
{
...
bool CzDENTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzDENTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzDENTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzDENTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
CoinWitnessData* CzDENTracker::GetSpendCache(const uint256& hashStake)
{
...
bool CzDENTracker::ClearSpendCache()
{
    AssertLockHeld(cs_spendcache);
...
std::vector<uint256> CzDENTracker::GetSerialHashes()
{
...
CAmount CzDENTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzDENTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzDENTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzDENTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzDENTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzDENTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzDENTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzDENTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzDENTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzDENTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzDENTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzDENWallet* zDENWallet)
{
    bool iszDENWalletInitialized = (NULL != zDENWallet);
    CMintMeta meta;
    ...
    if (! iszDENWalletInitialized)
        zDENWallet = new CzDENWallet(strWalletFile);
    meta.isSeedCorrect = zDENWallet->CheckSeed(dMint);
    if (! iszDENWalletInitialized)
        delete zDENWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzDENTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzDENTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzDENTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzDENTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzDENTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzDENTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzDENWallet* zDENWallet = new CzDENWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            if (fExcludeV1 && dMint.GetVersion() < 2)
                continue;
            Add(dMint, false, false, zDENWallet);
        }
        delete zDENWallet;
        LogPrint("zero", "%s: added %d dzden from DB\n", __func__, listDeterministicDB.size());
    }
